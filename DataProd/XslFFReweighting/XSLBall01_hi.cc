...
#include "XslFFReweighting/XSLBall01_hi.hh"
#include "CLHEP/Vector/LorentzVector.h"
using std::cout;
using std::endl;
using std::string;

XSLBall01_hi::XSLBall01_hi( double mB, double mXu, double q2, double theta_l, string mode) : 
  XSLPseudoScalarFF(mB,mXu,q2,theta_l) 
{  SetNormalizations(mode);  Compute(); }

XSLBall01_hi::XSLBall01_hi( XSLKin* DecayKin, string mode) : XSLPseudoScalarFF(DecayKin) 
{  SetNormalizations(mode);  Compute(); }

XSLBall01_hi::XSLBall01_hi( HepLorentzVector BLab, HepLorentzVector LepLab, HepLorentzVector XuLab, string mode) : XSLPseudoScalarFF( BLab, LepLab, XuLab) 
{  SetNormalizations(mode);  Compute(); }

double
XSLBall01_hi::GetFplus(double q2)
{
  double z=q2/(5.2794*5.2794);
  double LCSR=0;
  if(q2<14.3) LCSR=(0.206/(1-(2.34*z)+(1.77*z*z)));
  else{ 
    double mBs2=28.4;
    double zp=q2/mBs2;
    LCSR=(0.384/(1-zp));
...
void
XSLBall01_hi::SetNormalizations(string mode)
{
  ...
  if(mode=="Ignore") { _FLATQ2Normalization=1.0; _PHSPNormalization=1.0; _ISGW2Normalization=1.0; }
    ...
    cout<<"XSLBall01_hi::SetNormalizations  -- This model is not intended to be used with mode: "<<mode<<" !!!!"<<endl;
    ...
    cout<<"XSLBall01_hi::SetNormalizations  -- Unknown mode: "<<mode<<" !!!    Normalizations set to 1..."<<endl;
    _FLATQ2Normalization=1.0; _PHSPNormalization=1.0; _ISGW2Normalization=1.0;  }
